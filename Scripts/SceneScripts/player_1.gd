...
func _physics_process(delta):
	...
	var input_axis = Input.get_axis("p1_move_left", "p1_move_right")
...
func jump():
	...
	if jump_count < jump_max: 
		if is_on_floor() or form == "SPECTER":
			...
			if Input.is_action_just_released("p1_move_up") and velocity.y < JUMP_VELOCITY / 2:
				velocity.y = JUMP_VELOCITY / 2
				jump_count += 1
				$Jump.play()
			if Input.is_action_just_pressed("p1_move_up") and not just_wall_jumped:
